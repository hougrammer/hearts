...
const {hand, bestFreeCardsToPlay, round} = require('./config');
const otherPlayers = {};
...
let cardsToBeat = cardsPlayed.filter((element) => {
return returnOnlySameSuit(element, suit);
});
...
let viableCards = hand.filter((element) => {
return returnOnlySameSuit(element, suit);
});
...
let possibleChoices = allDifferences.filter((element) => {
return element.diff > 0;
});
...
let p1 = new OtherPlayer('p1');
let p2 = new OtherPlayer('p2');
let p3 = new OtherPlayer('p3');
